  ...
  <div class="card">
    ...
    <div class="card-header custom-card-header">
      <div class="float-right" v-if="hasAnyAnswerValue">
        <button
          @click="toggleView()"
          class="card-title btn btn-primary ml-4 view-button"
          :title="isList ? 'Voir les documents' : 'Voir les questionnaires'"
        >
...
Finalisé : l'instruction des pièces déposées est achevée"
          ...
          <tr v-for="questionnaire in accessibleQuestionnaires" :key="'questionnaire-' + questionnaire.id">
            ...
            <td class="end-date-column">
              <div v-if="questionnaire.end_date">
                <small>{{ formatDate(questionnaire.end_date) }}</small>
              </div>
            </td>
            ...
            <td class="w-1 action-column">
              <!-- Pour utilisateurs non-demandeur -->
              ...
              <!-- Pour demandeur -->
              <template v-else>
                <!-- Rédacteur = utilisateur courant et brouillon -->
                <template
                  v-if="
                ...
                <!-- Brouillon mais pas rédacteur -->
                <template
                  v-else-if="questionnaire.is_draft && questionnaire.editor && questionnaire.editor.id !== user.id"
                >
                  <div class="text-right">
                    <a
                      :href="questionnaireDetailUrl(questionnaire.id)"
                  ...
                  <div class="text-right">
                    <div class="btn-group">
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        ...
                        <button class="dropdown-item" type="button" @click="exportControl(questionnaire.id)">
    ...
    <div v-if="currentView === 'tree'">
      <questionnaire-tree-view :control="control" />
    </div>
...
/* Modifications principales :
   - defineComponent (Vue 3)
   - suppression du filter (transformé en méthode formatDate)
   - suppression de Vue.use(Vuex) (le store doit être appliqué sur l'app)
   - corrections de balises manquantes/fermetures dans le template
*/
  ...
  methods: {
    formatDate(date) {
      ...
      try {
        ...
        // En cas d'erreur, renvoyer la date brute
        return date
      }
    },
    ...
    showModal(qId) {
      ...
      // Note : utilisation de jQuery/bootstrap modal comme avant
      ...
      if (this.checkedCtrls.length) {
        ...
        destCtrls.forEach((ctrl) => {
          ...
          let newQ = {
            ...curQ,
            control: ctrl.id,
            questionnaire_files: curQ.questionnaire_files,
            is_draft: true,
            is_replied: false,
            is_finalized: false,
            id: null,
