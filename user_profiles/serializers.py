...
def resolve_keycloak_user_id(
    keycloak_admin: KeycloakAdmin, username_or_email: str
) -> str | None:
    ...
    def create(self, validated_data):
        ...
        if settings.KEYCLOAK_ACTIVE:
            ...
            role = keycloak_admin.get_client_role(
                ...
                role_name=UserProfile.INSPECTOR,
            )
        ...
        if profile:
            if settings.KEYCLOAK_ACTIVE:
                user_id_keycloak = resolve_keycloak_user_id(
                    keycloak_admin, user_data["username"]
                )
                if user_id_keycloak:
